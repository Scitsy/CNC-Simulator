...
namespace FanucSimulator
{
    ...
    public static class GCodeReference
    {
        ...
        public static readonly List<SetupTip> SetupTips = new()
        {
            ...
            new SetupTip
            {
                Title = "Auxiliary M-Codes (from this machine's own manual)",
                Body = "This simulator is modelled on a LEADWELL LTC-208 (serial L2TAG0843, built 2020) running a FANUC 0i-TF Plus, and the auxiliary M-codes below are transcribed from that machine's own operation manual, section 4-1. FANUC defines the common codes - M00/M01/M02/M30 program control, M03/M04/M05 spindle, M08/M09 coolant, M98/M99 subprograms - and those are the same on every FANUC lathe. Everything else is implemented in the machine BUILDER's PMC ladder rather than in the CNC, so no FANUC manual defines them and they differ from builder to builder. The ones this machine actually has include: M07 coolant through spindle, M10/M11 chuck clamp/unclamp, M12/M13 quill out/in, M14/M15 parts catcher extend/retract, M17/M18 auto door close/open, M19/M20 spindle orientation on/off, M21/M22 DOOR INTERLOCK BYPASS on/off, M23/M24 chamfering, M25-M28 bar feeder, M29 rigid tapping, M31/M32 chuck bypass, M33/M34 block skip, M37/M38 chip conveyor CW/stop, M47/M48/M49 soft limits, M51/M52 error detect, M53/M54 steady clamp, M55/M56 tailstock clamp, M57/M58 spindle air blow, M63/M64 chuck low pressure, M67 chip clean, M68 steady rest clean, M97 parts counter. Codes the manual marks as not fitted on this machine - the sub-spindle, Cs-axis, live tooling, Y axis and push-bar codes - still raise an alarm here, because commanding one on a machine that has no such equipment is a programming error. NOTE ON THE OLD GUESSES: before the manual was available this simulator shipped an inferred table, and three of those guesses were wrong. The worst was M21/M22, guessed as a parts catcher when they are really the door interlock bypass. If you wrote anything against the old table, re-check it. The codes are not modelled as machine behaviour here - this engine has no chuck, tailstock, conveyor or catcher state - they are accepted and logged so a real program runs without false alarms."
            },
            new SetupTip
            {
